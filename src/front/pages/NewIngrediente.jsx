...
const NewIngrediente = () => {
  ...
  const UploadIngredientImage = async (e) => {
    ...
    try {
      ...
      if (data.secure_url) {
        ...
        setImage(""); // limpio image si la URL viene de Cloudinary
      } else {
        console.error("❌ Error subiendo la imagen a Cloudinary");
      }
    } catch (error) {
      console.error("❌ Error en UploadIngredientImage:", error);
    }
  };
  ...
  function sendData(e) {
    ...
    if (!finalImageURL) {
      alert("⚠️ Debes ingresar o subir una imagen del ingrediente.");
      return;
    }
  ...
  return (
    <div>
      <form className="w-50 mx-auto" onSubmit={sendData}>
        ...
        {/* Imagen */}
        <div className="mb-3">
          <label className="form-label">Imagen (URL o subida)</label>
          <input
            value={urlImg || image}
